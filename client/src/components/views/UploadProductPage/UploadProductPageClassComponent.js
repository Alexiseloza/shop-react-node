...
export class UploadProductPage extends Component {
    ...
    onSubmit = (event) => {
        ...
        if (this.props.user.userData && !this.props.user.userData.isAuth) {
            return M.toast({html:' Please Login first' , classes:'#1e88e5 blue darken-1'})
        }

        if (!this.state.title || !this.state.description ||
            !this.state.continents || !this.state.images
            || !this.state.price) {
            return  M.toast({html: 'Please Check you Info!', classes:'rounded #ffa726 orange lighten-1'})
        }
        ...
        axios.post('/api/product/uploadProduct', variables)
            .then(response => {
                if (response.data.success) {
                    M.toast({html:'video Uploaded Successfully', classes: '#039be5 light-blue darken-1'})
                    setTimeout(() => {
                        this.props.history.push('/')
                    }, 1000);
                } else {
                    M.toast({html:'Failed to upload video' , classes:'#ffca28 amber lighten-1'})
                }
            })
    }
